...
TERM_TIMEOUT_SECONDS="${CONTINUUM_STOP_NOW_TERM_TIMEOUT_SECONDS:-5}"
...
write_force_stopped_state() {
  local interrupted="$1"
  python3 - "$STATUS_PATH" "$PROJECT_NAME" "$interrupted" <<'PY'
from __future__ import annotations
...
detail = "Immediate stop requested by operator."
if interrupted:
    detail = "Immediate stop requested by operator; escalated from TERM to KILL."
...
signal_service_kill() {
  if [[ -f "$LAUNCHD_SERVICE" ]] && command -v launchctl >/dev/null 2>&1; then
    launchctl kill KILL "$LAUNCHD_TARGET" >/dev/null 2>&1 || true
  fi
  if [[ -f "$SYSTEMD_SERVICE" ]] && command -v systemctl >/dev/null 2>&1; then
    systemctl --user kill --signal=KILL "$SYSTEMD_TARGET" >/dev/null 2>&1 || true
  fi
}
...
if pid_alive "$ACTIVE_CODEX_PID"; then
...
interrupted="false"
for ((i = 0; i < TERM_TIMEOUT_SECONDS; i++)); do
  if ! pid_alive "$ACTIVE_CODEX_PID" && ! pid_alive "$SUPERVISOR_PID"; then
    break
  fi
  sleep 1
done
...
cleanup_aux
rm -f "$PIDFILE"
write_force_stopped_state "$interrupted"

if [[ "$interrupted" == "true" ]]; then
  echo "Stopped '$PROJECT_NAME' now (escalated from TERM to KILL)"
else
  echo "Stopped '$PROJECT_NAME' now"
fi
